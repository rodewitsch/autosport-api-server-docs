...
export class GetOrganizerQueryDtoV1 {
  @ApiPropertyOptional({
...
@Exclude()
export class GetOrganizerResponseDtoV1 {
  @Expose()
  @ApiProperty({
  ...
  @Transform(({ value }) => value && value.length && value.map((type) => type.c_race_type))
  c_race_types: number[];

  constructor(partial: Partial<GetOrganizerResponseDtoV1>) {
