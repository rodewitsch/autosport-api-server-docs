...
import { GetEventResponseDtoV3 } from '../../../event/v3/dto/get-events.dto';
import { EventType } from '../../neutral/enums/event-type.enum';

export class GetChampionshipQueryDtoV3 extends PartialType(LimitOffsetQueryDto) {
  @ApiPropertyOptional({
...
@Exclude()
export class GetChampionshipEventResponseDtoV3 extends OmitType(GetEventResponseDtoV3, ['image'] as const) {
  constructor(partial: Partial<GetChampionshipEventResponseDtoV3>) {
    super(partial);
...
@Exclude()
export class GetChampionshipResponseDtoV3 {
  @Expose()
  @ApiProperty({
  ...
  @Transform(({ obj }) => obj.image && `/api/v3/championships/${obj.championship_id}/image?d=${obj.updated_at}`)
  image: boolean;
  ...
  @ApiProperty({
    ...
    type: [GetChampionshipEventResponseDtoV3],
  })
  @Transform(({ value }) => (value ? value.map((event) => new GetChampionshipEventResponseDtoV3(event)) : []))
  events: GetChampionshipEventResponseDtoV3[] = [];

  constructor(partial: Partial<GetChampionshipResponseDtoV3>) {
    Object.assign(this, partial);
  }
}
