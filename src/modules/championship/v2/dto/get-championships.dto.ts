...
import { GetEventResponseDtoV2 } from '../../../event/v2/dto/get-events.dto';
import { EventType } from '../../neutral/enums/event-type.enum';

export class GetChampionshipQueryDtoV2 extends PartialType(LimitOffsetQueryDto) {
  @ApiPropertyOptional({
...
@Exclude()
export class GetChampionshipEventResponseDtoV2 extends OmitType(GetEventResponseDtoV2, ['image'] as const) {
  constructor(partial: Partial<GetChampionshipEventResponseDtoV2>) {
    super(partial);
...
@Exclude()
export class GetChampionshipResponseDtoV2 {
  @Expose()
  @ApiProperty({
  ...
  @Transform(({ obj }) => obj.image && `/api/v2/championships/${obj.championship_id}/image?d=${obj.updated_at}`)
  image: boolean;
  ...
  @ApiProperty({
    ...
    type: [GetChampionshipEventResponseDtoV2],
  })
  @Transform(({ value }) => (value ? value.map((event) => new GetChampionshipEventResponseDtoV2(event)) : []))
  events: GetChampionshipEventResponseDtoV2[] = [];

  constructor(partial: Partial<GetChampionshipResponseDtoV2>) {
    Object.assign(this, partial);
  }
}
